...
static float xInc = 1.0f;
static float yInc = 1.0f;
static float zInc = 1.0f;
...
void drawQuad(int a, int b, int c, int d, float scale)
{
   glBegin(GL_QUADS);
   glVertex3f(scale * vertices[a][0], scale * vertices[a][1], scale * vertices[a][2]);
   glVertex3f(scale * vertices[b][0], scale * vertices[b][1], scale * vertices[b][2]);
   glVertex3f(scale * vertices[c][0], scale * vertices[c][1], scale * vertices[c][2]);
   glVertex3f(scale * vertices[d][0], scale * vertices[d][1], scale * vertices[d][2]);
   glEnd();
}

void drawCube(float scale)
{
   drawQuad(1, 0, 3, 2, scale);
   drawQuad(2, 3, 7, 6, scale);
   drawQuad(3, 0, 4, 7, scale);
   drawQuad(6, 5, 1, 2, scale);
   drawQuad(4, 5, 6, 7, scale);
   drawQuad(5, 4, 0, 1, scale);
}
...
void drawScene(void)
{
   ...
   glPushMatrix();
   glTranslatef(-30.0, 10.0, -20.0); // Translate the shadow to the back wall.
   glScalef(0.5, 0.5, 0.0); // Collapse z-values to make a 2D shadow.
   ...
   glPushMatrix();
   glTranslatef(30.0, 0.0, -15.0); // Translate the shadow to the back wall.
